    ...
    do while ((conf<=max_confor).and.isReadGood)
        ...
        if(isReadGood) then ! read was succesfull  
            ...
            case ("cubic")
                ...
                ! chain_rot that has NOT applied pbc 

                ! Q1? for overlap maybe use chain_pbc combined with subroutine maximum_xnucl(conf,isVolfracLargerOne)
                ! Q2> Check usage of isVdW ???
...
! Finds minimal z-value of  object chain_elem_index

subroutine find_zminimum_chain_elem_index(nseg,nelem,chain_elem_index,rmin)
    ...
    do k=1,3
        rmin(k)=chain_elem_index(k,1)%elem(1)
    enddo
    ...
    do s=1,nseg              ! loop over segments
        do j=1,nelem(s)      ! loop over number of chain elements for segment s 
            ztemp=chain_elem_index(3,s)%elem(j)
            if(ztemp<=zmin) then 
                zmin=ztemp
                do k=1,3
                    rmin(k)=chain_elem_index(k,s)%elem(j)
                enddo 
            endif    
        enddo 
    enddo  
