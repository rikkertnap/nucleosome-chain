...
Uses the molecular theory approach to compute the structure of a nucleosome n-mer in 3D (cartesian and oblique) coordinates. 
The aqueous solution is characterized by pH, NaCl, KCk and Ca2Cl.
The nucleoeosome chains are obteaind from simulation from the 1CPN-model. This trajectory are inverted adn mapped back to amino-acid 
unit resolution of the 3SPN-model.
Formation of Ca-bridges between phosphate monomer is taken into account
Program has inherited features and input varialbles of srcf90-fdis5-3D, which solved in 3D a tethered polyelectroltye layer of arbitrair composition. 
