...
contains
    ...
    subroutine fcnnucl_Mg(x,f,nn)
        ...
        use parameters, only : vsol,vNa,vK,vCl,vFe2,vFe3,vCa,vMg,deltavAA,vnucl,vPP
        ...
        use field, only      : xsol,xNa,xCl,xK,xHplus,xOHmin,xFe2,xFe3,xMg,xCa,rhopol,rhoqpol,rhoq
        use field, only      : psi,gdisA,gdisB,fdis,rhopol_charge, fdisPP, fdisP2Mg, fdisP2Fe2, rhoqphos
        ...
        real(dp) :: pro,lnpro
        integer  :: n,i,j,k,c,s,t,jcharge,kr,m,mr           ! dummy indices
        integer  :: ind, k_ind, m_ind 
        integer  :: JJ, KK
    ...
    subroutine compute_average_charge_PP(avfdisP2Mg,avfdisP2Fe2,avfdisPP)
        ...
        use volume, only     : volcell, inverse_indexneighbor_phos
        use chains, only     : indexconf, type_of_monomer, logweightchain, nelem, ismonomer_chargeable
        ...
        real(dp) :: lnexppivw(nsize)
        real(dp) :: pro,lnpro
        integer  :: n,i,j,k,c,s,kr,m,mr,t,jcharge     ! dummy indices
        integer  :: k_ind, m_ind
        integer  :: JJ, KK
        real(dp) :: local_avfdisP2Mg,local_avfdisPP(7,7),local_avfdisP2Fe2
        real(dp) :: sumrhopairs
        real(dp) :: K0aPP                                ! Kdis of P2Mg pair temporarily define 
        integer  :: nsizepsi
    ...
    subroutine compute_FEchem_react_PP(FEchemPP)
        ...
        use precision_definition
        use globals, only    : nsize, nsegtypes, nseg,local_conf, DEBUG
        use parameters, only : vsol,vnucl, vPP,qPP,K0aAA,Phos,Phos2Mg, Phos2Fe2, ta  
        use volume, only     : nx, ny
        use volume, only     : inverse_indexneighbor_phos
        use chains, only     : indexconf, type_of_monomer, logweightchain, nelem, ismonomer_chargeable
        use chains, only     : type_of_charge, elem_charge, indexconfpair, nneigh
        use chains, only     : inverse_index_phos, energychainLJ, no_overlapchain
        use field, only      : xsol,psi,fdis, fdisPP, fdisP2Mg, fdisP2Fe2
        use field, only      : q, lnproshift
